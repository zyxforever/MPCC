...
def main():
    ...
    class Center_MemoryBank(object):
        ...
        def update(self, features,targets):
           ...
           # print('zguxubg')
            ...
            #print(targets)
           # print(self.ptr)
            self.targets[self.ptr:self.ptr + b].copy_(targets)
            #print(self.targets)
            self.ptr += b
        def get_label(self):
            return self.targets
    ...
    if os.path.exists(p['end2end_checkpoint']):
        ...
        model.cuda()
        start_epoch = 50 # 10000 for evaluate directly
    else:
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        if epoch <= 50:
            print('Train pretext...')
            gcc_train(cm,p,train_dataloader, model, criterion1, criterion2, optimizer,
        ...
        if epoch > 0 and epoch % 5 == 0:
            ...
            if epoch % p['save_iterval'] ==0:
                torch.save({'optimizer': optimizer.state_dict(), 'model': model.state_dict(),
                        'epoch': epoch + 1}, f"{p['end2end_checkpoint']}_{epoch}")
        # Update memory bank
